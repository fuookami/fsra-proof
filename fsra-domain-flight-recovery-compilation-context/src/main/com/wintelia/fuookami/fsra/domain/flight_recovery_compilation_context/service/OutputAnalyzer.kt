...
class OutputAnalyzer(
    ...
    private fun analyzeFlightSolution(recoveryPlan: RecoveryPlan, recoveryedFlights: List<FlightTask>, canceledFlights: List<FlightTask>): Result<List<RecoveryedFlightDTO>, Error> {
    ...
    private fun analyzeMaintenanceSolution(recoveryPlan: RecoveryPlan, recoveryedMaintenances: List<FlightTask>, canceledMaintenances: List<FlightTask>): Result<List<RecoveryedMaintenanceDTO>, Error> {
        ...
        for (recoveryedMaintenance in recoveryedMaintenances) {
            when (val ret = dumpMaintenance(recoveryPlan, recoveryedMaintenance, false)) {
                is Ok -> { maintenances.add(ret.value) }
                is Failed -> { return Failed(ret.error) }
            }
        }
    ...
    private fun dumpFlight(recoveryPlan: RecoveryPlan, flight: FlightTask, canceled: Boolean): Result<RecoveryedFlightDTO, Error> {
        return Ok(RecoveryedFlightDTO(
            recoveryPlanId = recoveryPlan.id,
            id = flight.actualId,
            no = if (flight.type is FlightFlightTask) { (flight as Flight).plan.no } else { "" },
            oldAcType = if (flight.originTask.aircraft != null) { flight.originTask.aircraft!!.minorType.code } else { null },
            newAcType = if (canceled) { null } else { flight.aircraft!!.minorType.code },
            oldAcReg = if (flight.originTask.aircraft != null) { flight.originTask.aircraft!!.regNo } else { null },
            newAcReg = if (canceled) { null } else { flight.aircraft!!.regNo },
            date = flight.time?.let { Date(it.begin) }.toString(),
            std = flight.scheduledTime?.begin?.toString(),
            sta = flight.scheduledTime?.end?.toString(),
            etd = if (canceled) { null } else { flight.time?.begin?.toString() },
            eta = if (canceled) { null } else { flight.time?.end?.toString() },
            canceled = canceled,
            ...
            delayed = if (canceled) { false } else { flight.delay != Duration.ZERO },
            advanced = if (canceled) { false } else { flight.advance != Duration.ZERO },
            aircraftChanged = if (canceled) { false } else { flight.aircraftChanged },
            aircraftMinorTypeChanged = if (canceled) { false } else { flight.aircraftMinorTypeChanged }
        ))
    }
    ...
    private fun dumpMaintenance(recoveryPlan: RecoveryPlan, maintenance: FlightTask, canceled: Boolean): Result<RecoveryedMaintenanceDTO, Error> {
        return Ok(RecoveryedMaintenanceDTO(
            recoveryPlanId = recoveryPlan.id,
            ...
            estimatedBeginTime = if (canceled) { null } else { maintenance.time?.begin.toString() },
            estimatedEndTime = if (canceled) { null } else { maintenance.time?.end.toString() },
            estimatedAirport = if (canceled) { null } else { maintenance.dep.icao },
            canceled = canceled,
            delayed = if (canceled) { false } else { maintenance.delay != Duration.ZERO },
            airportChanged = if (canceled) { false } else { maintenance.routeChanged }
        ))
    }
}
